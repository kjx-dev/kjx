...
export function getOrderedCategories(catTiles = [], groups = []) {
  ...
  if (Array.isArray(catTiles) && catTiles.length > 0) {
    ...
    categoriesToSort = groups.map(g => {
      // Match tile by case-insensitive name comparison
      const tileMatch = catTiles.find(t => {
        const tileKey = (t.k || '').toLowerCase().trim()
        const tileLabel = (t.label || '').toLowerCase().trim()
        const parentName = (g.parent.name || '').toLowerCase().trim()
        return tileKey === parentName || tileLabel === parentName
      })
      const baseLabel = tileMatch ? (tileMatch.shortLabel || tileMatch.label) : g.parent.name
      return {
        k: g.parent.name, // Use actual database name, not tile key
        label: g.parent.name,
        ...
        name: g.parent.name // Ensure name property is set
      }
    })
  } else {
    return []
